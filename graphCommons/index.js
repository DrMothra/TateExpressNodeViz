...
GraphCommons.prototype.delete_graph = function(_id, _callback) {

	if(!_id) throw new Error('missing graph id');

	unirest.delete(api_url + 'graphs/' + _id)
		.headers( {'Authentication': api_key} )
		.end(function(response) {
			if(_callback) _callback(response.body);
		});
...
GraphCommons.prototype.search = function(_search_obj, _callback) {
	...
	unirest.get(api_url + 'search' + '?query=' + _search_obj.query + '&limit=' + _search_obj.limit)
	.headers({'Authentication': api_key})
	.headers({'Content-Type' : 'application/json'})
	.send({})
	.end(function (response) {
